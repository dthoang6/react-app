...
function HomeGuest() {
  ...
  const initialState = {
    username: {
      value: "",
      hasErrors: false,
      message: "", //an actual error message to do flash message output
      isUnique: false, //if the username is unique or not
      checkCount: 0 //when we increment it, we can watch it for changes to go run an Axios request to see if the username is unique or not.
    },
    email: {
      value: "",
      hasErrors: false,
  ...
  function ourReducer(draft, action) {
    switch (action.type) {
      case "usernameImmediately":
        draft.username.hasErrors = false //assume there is no error for every time
      ...
      case "usernameAfterDelay":
        if (draft.username.value.length < 3) {
          draft.username.hasErrors = true
          draft.username.message = "Username has to be more than 3 characters."
        }
  ...
  //setup delay timer for waiting username type before check errors
  useEffect(() => {
  ...
  //setup delay timer for waiting email type before check errors
  useEffect(() => {
  ...
  //setup delay timer for waiting password type before check errors
  useEffect(() => {
  ...
  //send off Axios to check username unique
  useEffect(() => {
  ...
  //send off Axios to check email unique
  useEffect(() => {
  ...
  function handleSubmit(e) {
    e.preventDefault()
    //call all of our different rules
